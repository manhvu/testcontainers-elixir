    ...
    adapter = fn request ->
      ...
      case Map.get(stubs, {method, url}) do
        {status, body} when is_binary(body) ->
          response = Req.Response.new(status: status, body: body, headers: [{"content-type", "text/plain"}])
          {request, response}
        ...
        nil ->
          response = Req.Response.new(status: 404, body: "not found", headers: [{"content-type", "text/plain"}])
          {request, response}
      end
    end
